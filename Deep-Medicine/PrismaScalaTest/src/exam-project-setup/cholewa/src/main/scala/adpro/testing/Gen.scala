...
case class Gen[A] (sample :State[RNG,A]) {
  ...
  def listOfN (n :Int) :Gen[List[A]] =
    Gen(sample.map(x => List.fill(n)(x)))
...
object Gen {
  ...
  def choose (start:Int, stopExclusive:Int):Gen[Int] =
    Gen(State(nonNegativeInt).map(x => x % (stopExclusive - start) + start))
  ...
  def boolean :Gen[Boolean] = Gen(State(nonNegativeInt).map((((x:Int) => x % 2 == 0))))

  // Hint: Recall from Exercise1.scala that we already implemented a random
  // number generator for doubles.

  def double :Gen[Double] = Gen(State(_double))
  ...
  def weighted[A](g1: (Gen[A],Double), g2: (Gen[A],Double)): Gen[A] = {
    val c = g1._2 / (g1._2 + g2._2)
    double.flatMap(d => if(d < c) g1._1 else g2._1)
  }
