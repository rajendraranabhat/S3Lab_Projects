...
//
// AUTHOR1: Al Aqtash, Ahmed - ahaq@itu.dk
// AUTHOR2:
//
...
object Exercises extends App {
  ...
  def power(x: Double, n: Int) : Double = {
    def helper(r: Double, n: Int) : Double = {
      if (n == 0) r
      if (n > 0)
        if (n % 2 == 0) helper (r * r, n / 2)
        if (n % 2 != 0) helper (r * r, n - 1)
      if (n < 0) helper (r * (-1.0), n)
    }
    helper(x, n)
  }
  ...
  // Exercise 4
   ...
   def fib (n: Int) : Int = {
     @tailrec
     def _fib (n: Int, next: Int, sum: Int) : Int = {
       if (n == 0) sum
       else _fib(n - 1, sum + next, next)
     }
     _fib (n, 1, 0)
   }
