...
def load_labs_data(mf):
    read_var = ['Account', 'Collect Date','TestCode','Result']
    df = pd.read_csv(path_csv+'fake_lab_data.csv', usecols=read_var,skipinitialspace=True)

    AccountId = []
    labs =[]
    for index, row in mf.iterrows():
        try:
            #match account and admitdate for lab.Process only the labs from admission date
            lab_rec = df[df['Account'] == row['Account']]
            sel_lab_rec = lab_rec[lab_rec['Collect Date']== row['Admission_Date']]
...
def process_labs (df):
     ...
     PROTUR_rec = df[df['Result'] != 'Missing']
