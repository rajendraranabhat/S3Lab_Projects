...
def load_meds_data(mf):
    df = pd.read_csv(path_csv+'fake_meds_data.csv', skipinitialspace=True)
    AccountId = []
    meds =[]
    ...
    for index, row in mf.iterrows():
        try:
            ...
            med_rec = df[df['Account'] == row['Account']]
...
def process_meds(df):
    ...
    if(len(sel_df) == 0):
        ...
        #We dont need them all for this paper, but putting up the code for all just in case required for future
