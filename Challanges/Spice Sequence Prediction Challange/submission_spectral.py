...
#set parameter rank value (the estimated rank of the Hankel matrix)
rank=17

# name of this submission (no space or special character)
name = "rank_" + str(rank) + "_sparse_" + "classic" + "_lrows_lcolumns_" + str(5)
...
def learn(train_file, parameter):
    ...
    A = S_app.LearnAutomaton(rank=parameter, lrows=5, lcolumns=5, version="classic",
                         partial=True, sparse=True)
